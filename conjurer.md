...
The practical corollary is to **separate the deterministic plan from the generative act, and verify the first before risking the second.** Wherever an operation has a conserved, checkable half and a lossy, irreversible half, Conjurer pulls them apart rather than fusing them. Materialisation is the clearest case: resolving references and ordering dependencies loses nothing and can be checked, so it runs as a preflight and produces an inspectable plan; turning that plan into code is lossy and non-deterministic, so it runs only against a plan already known to be sound. The same shape recurs in a workflow's dry run, in exhumation grading its confidence before it emits, in a certainty contract checked before a result is trusted. The discipline is always the same: do the part that can be done without loss, confirm it, and only then incur the part that cannot be undone. Generating against an unverified plan is the code-generation form of inventing a rationale — a confident artefact on an unsound foundation — and the remedy is structural, not careful: make the verifiable phase a separate, mandatory step.
